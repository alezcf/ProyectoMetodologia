...
const DeleteUser = () => {
  ...
  const handleDeleteUser = async () => {
    try {
      const response = await axios.delete("/rol/eliminarusuario", { data: { rut } });
  ...
  return (
    ...
    <Container className={classes.formContainer}>
      <Grid >
        <Grid container justify="center" item xs={5} sm={8} md={6} lg={4}>
          <div className={classes.formContent}>
            <Typography variant="h5" align="center" gutterBottom className={classes.title}>Eliminar Usuario</Typography>
      ...
      <Button className={classes.formButton} fullWidth variant="contained" type="submit">Ingrese para continuar</Button>
            ...
            {error && (<Alert severity="error" style={{ marginTop: "10px" }}><AlertTitle>Error</AlertTitle>
            {error}
            </Alert>)}
          </div>
        </Grid>
